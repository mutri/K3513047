 ...
 function update()
 {
 	...
 	if (!$id)
 	{
		...
		foreach ($query->result_array() as $data)
		{
			...
			$this->table->add_row($no, $nim, $nama, $prodi, $angkatan, $ttl, $jk, $alamat, $peminatan, anchor('coba/update?id='.$data['nim'],"<button class='btn btn-success'>Edit</button>"));
		}
	...
	else
	{
		...
		if ($update)
		{
			$nim = strtoupper($this->input->post('nim', TRUE));
 			...
 			echo "<script>alert('Data Berhasil di Update'); window.location='".base_url('coba/update')."'</script>";
 ...
 function delete()
 {
 	...
 	if (!$id)
 	{
		...
		foreach ($query->result_array() as $data)
		{
			...
			$this->table->add_row($no, $nim, $nama, $prodi, $angkatan, $ttl, $jk, $alamat, $peminatan, "<a href='".base_url('coba/delete?id='.$nim.'')."' class='btn btn-sm btn-danger'>Hapus</a>");
		}
		$tmpl = array ( 'table_open'  => '<table class="table table-striped table-bordered table-hover">' );
	...
	else if ($id)
	{
		...
		if ($jum>0)
		{
			...
			echo "<script>alert('Data Berhasil di Hapus'); window.location='".base_url('coba/delete')."'</script>";
		}
		else
		{
			// redirect(base_url('coba/select'));
 ...
 function select()
 {
	...
	if (!$id)
	{
		...
		foreach ($query->result_array() as $data)
		{
			...
			$this->table->add_row($no, $nim, $nama, $prodi, $angkatan, $ttl, $jk, $alamat, $peminatan, anchor('coba/select?id='.$data['nim'],"<button class='btn btn-primary'>Detail</button>"));
		}
		$tmpl = array ( 'table_open'  => '<table class="table table-striped table-bordered table-hover">' );
